...
interface EnergyToken {
    ...
    function updateCoolDownSettings(bool _enabled, uint256 _timeInSeconds) external;
}
...
contract TokenVesting is Ownable {
    ...
    function token() public view virtual returns (EnergyToken) {
        return _token;
    }
    ...
    function getReleasableAmount(address beneficiary) public view returns (uint256) {
        ...
        uint256 releasableAmount = _releasableAmount(beneficiary, phases);
    ...
    function addTokensToVesting(address beneficiary, uint256 amount) external onlyOperatorOrOwner {
        ...
        if (_lockDurations[0] == 0 && block.timestamp >= _startTime) {
    ...
    function releaseAll() public onlyOwner {
        _token.updateCoolDownSettings(false, 0);
            ...
            if (block.timestamp >= releaseTime) {
                for (uint256 j = 0; j < _beneficiaries.length; j++) {
                    ...
                    if (releasableAmount > 0) {
                        _releasedAmount[beneficiary] = _releasedAmount[beneficiary].add(releasableAmount);
                        IERC20(address(_token)).transfer(beneficiary, releasableAmount);
        ...
        _token.updateCoolDownSettings(true, 30);
    }

    function releaseInRange(uint256 startIndex, uint256 endIndex) public onlyOwner {

        _token.updateCoolDownSettings(false, 0);

        require(endIndex <= _beneficiaries.length, "TokenVesting: endIndex out of range");
    ...
    function returnUnsoldTokens(address to) external onlyOwner {
        uint256 contractTokenBalance = IERC20(address(_token)).balanceOf(address(this));
        require(contractTokenBalance > 0, "TokenVesting: no unsold tokens");
        IERC20(address(_token)).transfer(to, contractTokenBalance);
    }
}